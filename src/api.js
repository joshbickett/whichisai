...
export const getAIImage = async (search) => {
  return new Promise(async (resolve, reject) => {
    ...
    const url = `https://lexica.art/api/v1/search?q=$image of ${search}`;
...
export const getNormalImages = async (search) => {
  ...
  const url = `https://api.unsplash.com/search/photos?query=${search}&client_id=${key}&content_filter=high&per_page=30`;
